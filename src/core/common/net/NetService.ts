...
interface $gmType {
    ...
    showNSLogData(data?: string, ...cmds: number[]);
    /**
     * 使用黑名单模式，进行输出
     * 
     * @param {...number[]} cmds
...
$gm.showNSLogData = (data?: string, ...cmds) => {
    let nsFilter = $gm.__getNSFilter(...cmds);
    let output = [];
    $gm.nsLogs.forEach(log => {
        if ($gm.__nsLogCheck(log, nsFilter)) {
            if (data) {
                if (log.data[data]) {
                    for (let i of log.data[data]) {
                        output.push(i);
                    }
                }
            } else {
    ...
    export abstract class NetService {
        ...
        */
        ...
        /**
             ...
             * @memberOf $gmType
             */
        protected $writeNSLog: { (time: number, msgType: string, type: "send" | "recv", cmd: number, data?: any): void };
        ...
        constructor() {
            ...
            this.$writeNSLog = (time, msgType, type, cmd, data) => {
                data = data == undefined ? "" : JSON.parse(JSON.stringify(data));
                let log = doFreeze({ time, msgType, type, cmd, data });
                ...
                if (($gm.__nsLogCheck(log, nsFilter) && Global.isDebug) || (DEBUG)) {
         ...
         * @memberOf NetService
         */
        public maxConTime = Time.FIVE_MINUTE;
        ...
        protected decodeBytes(bytes: ByteArray) {
            let recieveMSG = this._receiveMSG;
            ...
            out: while (true) {
                ...
                if (bytes.readAvailable >= msgLength) {
                    ...
                    if (cmd != 1002) {
                        this.$writeNSLog(now.getTime(), type + "", "recv", cmd, data);
                    }
